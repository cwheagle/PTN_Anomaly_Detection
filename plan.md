...
- **작업 내용:**
  ...
  - **추세 분석 기초 (Trend Analysis) (Complete)**: 이상 점수의 상승 기울기(Slope) 분석을 통한 초기 단계 장애 징후 포착 및 중복 저장 방지 필터링 구현.
  - **잔여 수명 예측 기초 (RUL Prediction) (Go)**: 현재 추세 지속 시 장애 발생 예상 시점(Time to Failure)을 산출하여 운영자 가이드 제공.
